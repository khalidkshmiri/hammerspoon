...
Hyper = **Left Cmd + Left Ctrl + Left Opt + Left Shift** held simultaneously.

If you use [Karabiner-Elements](https://karabiner-elements.pqrs.org), you can map Caps Lock (or another key) to all four modifiers at once for a single-key Hyper. The script handles the small timing gap that remappers can introduce between the click event and the modifier flags.
...
|---|---|---|
| `RESIZE_MARGIN` | `20` px | Distance from edge where Hyper+drag triggers resize instead of move |
